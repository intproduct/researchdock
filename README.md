...
- 项目修订历史：每次保存落一条只追加的完整快照，详情页可按时间倒序展开查看与翻页。
...
**尚未实现**：GitHub 实时接入、不同设备间直接历史比较、自动同步/合并、文件备份、非 Git 目录、实验和文献集成、撤销/恢复历史版本、研究日志（当前历史仅为项目状态快照，非完整日志）。这一版不上传文件正文，不执行 fetch/push。
