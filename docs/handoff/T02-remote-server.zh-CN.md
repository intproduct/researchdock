...
这是一份操作说明，审计者尚未连接或配置用户服务器。截至 2026-09-20，用户已验证远端 Docker/Compose 和四个基础镜像拉取（准备阶段 1–4）；T02 第四轮审计已关闭全部已知代码问题，见 [审计报告](reports/T02-review-round4.md)。现在可从第 5 节开始做远端验收（5–8），整体尚未 accepted。不要上传半成品、根 .env、开发数据库、.venv 或 node_modules。
...
不用先建立 GitHub 仓库。下面只打包已提交且被 Git 跟踪的文件，不带开发环境和凭据。代码修复已交付并复审；执行时仍须确认工作区干净，并记录实际打包的提交 SHA。
